    ...
    for line in lines[1:]:
        ...
        if multiline:
            ...
            if len(split_line) == 2:
                ...
                if split_line[1].strip() == "ind" or split_line[1].strip() == "grp":
                    split_line[1] = "trg"
                tweets.append([id_builder, line_builder, split_line[1].strip()])
                multiline = False
            continue
        if split_line[3].strip() == "ind" or split_line[3].strip() == "grp":
            split_line[3] = "trg"
...
if __name__ == "__main__":
    ...
    file.write("id\ttweet\tsubtask_b\n")
    for line in train:
        file.write(line[0] + "\t" + line[1] + "\t" + line[2] + "\n")

    file1 = open("../lib/test.tsv", "a")
    file2 = open("../lib/labelb.tsv", "a")
